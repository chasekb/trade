...
from ..web_handlers import BacktestHandlers
...
async def run_backtest(request: BacktestRequest, backtest_handlers: BacktestHandlers = None):
...
async def run_backtests_alias(request: BacktestRequest, backtest_handlers: BacktestHandlers = None):
    """Alias endpoint to run a backtest (parity with modular client)."""
    check_handlers_ready("backtest_handlers", backtest_handlers)
    return await backtest_handlers.run_backtest(request.dict())

@router.get("/api/backtest/results")
async def get_backtest_results(backtest_handlers: BacktestHandlers = None):
    """Get all backtest results."""
    check_handlers_ready("backtest_handlers", backtest_handlers)
    return await backtest_handlers.get_backtest_results()

@router.get("/api/backtest/history")
async def get_backtest_history(limit: int = 10, offset: int = 0, backtest_handlers: BacktestHandlers = None):
...
async def get_backtest(backtest_id: int, backtest_handlers: BacktestHandlers = None):
    """Get a specific backtest by ID."""
    check_handlers_ready("backtest_handlers", backtest_handlers)
    return await backtest_handlers.get_backtest(backtest_id)

@router.get("/api/backtest/stats")
async def get_backtest_stats(backtest_handlers: BacktestHandlers = None):
    """Get backtest statistics."""
    check_handlers_ready("backtest_handlers", backtest_handlers)
    return await backtest_handlers.get_backtest_stats()

@router.delete("/api/backtest/{backtest_id}")
async def delete_backtest(backtest_id: int, backtest_handlers: BacktestHandlers = None):
    """Delete a backtest."""
    check_handlers_ready("backtest_handlers", backtest_handlers)
    return await backtest_handlers.delete_backtest(backtest_id)

@router.get("/api/backtest/filters")
async def get_backtest_filters(backtest_handlers: BacktestHandlers = None):
