...
int main() {
  ...
  try {
    ...
    if (outputs.empty()) {
      std::cerr << "Transformer ONNX export produced no outputs"
                << std::endl;
      return 1;
    }

    if (outputs[0].GetTensorTypeAndShapeInfo().GetElementCount() == 0) {
      std::cerr << "Transformer ONNX export produced an empty output"
                << std::endl;
      return 1;
    }
  } catch (const std::exception &ex) {
