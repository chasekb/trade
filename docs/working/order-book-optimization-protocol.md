...
An exchange fill is one authoritative executed fill event (including a partial fill); a local fill is the equivalent paper/simulation settlement event; an opening leg is an entry-side fill allocation; a closing leg is an exit-side fill allocation; and a closed round trip is the attributed entry plus all required closing legs with a final realized outcome. These grains are not interchangeable. `n_fills` counts executed fill events (exchange fills for `live`, local fills for `live_parity_paper` and `simulated`), while `n_exchange_fills` counts authoritative exchange fills and is `null` when the mode has no exchange. `n_closed_round_trips` counts completed round trips. The objective metrics below use `n_closed_round_trips` as their denominator; lifecycle coverage and execution rates use the explicitly named lifecycle denominators.
...
Every group carries `n_rows`, `n_quotes`, `n_signals`, `n_intents`, `n_blocked`, `n_rejected`, `n_submitted`, `n_fills`, `n_exchange_fills`, `n_closed_round_trips`, and elapsed UTC window. `n_exchange_fills` is `null` outside `live`; `n_submitted` means exchange submission in `live` and the corresponding local settlement-attempt count in non-live modes. Pool monetary numerators before calculating ratios; never average per-symbol ratios to obtain an overall ratio. Include exposure and elapsed time so frequency comparisons are fair.
...
For live, use actual exchange fees and realized fill prices where available; missing actual components make the corresponding claim unavailable, not zero. For live-parity and simulation, use the declared modeled schedule and assumptions. Apply costs exactly once. Portfolio-level `total_fees` replaces a sum of per-trade fees; it is never added to that sum. Missing, non-finite, or unverified required fee, spread, or slippage components fail closed for actionability and promotion: the candidate receives `gate_status=fail`, cannot be selected, and may report only coverage or explicitly partial outcome fields.
...
A candidate is actionable only when all required cost components are present and finite, directional edge is positive, fee-adjusted edge is strictly greater than zero (equality fails), and strength clears the configured threshold. Negative cost inputs are clamped to zero for gating and reported as data warnings; missing costs are never treated as zero.
...
- `profit_factor = sum(net_pnl_i where net_pnl_i > 0) / abs(sum(net_pnl_i where net_pnl_i < 0))`; if no losses and wins exist, use `infinity` plus `no_losses`; if both are zero, use `null` plus `degenerate_no_pnl`;
...
| `n_exchange_fills` | integer/null | authoritative exchange fills; null outside live | group |
