...
- `fresh`: quote age <= 2,000 ms and valid bid/ask, mid, finite spread, and positive depth.
- `stale`: age > 2,000 ms and <= 5,000 ms; display with an explicit stale badge, never create a new live entry.
- `expired`: age > 5,000 ms, timeout, malformed/empty book, or failed request; no signal is executable.
- Account snapshot `fresh`: age <= 15,000 ms. `account_degraded`: >15,000 ms, failed, or valuation incomplete. No new live entry, add, or manual buy is allowed while degraded. Exits may be allowed only under a separately configured emergency policy using a still-authoritative holding quantity; otherwise block and show the reason.
...
- Every emitted metric/log includes the required context fields and contains no credential or signed-request material.
