...
class OrderBookStrategyValidator:
    def __init__(self, base_url="http://localhost:8000"):
        ...
        self.granularities = [3600, 21600, 86400]  # 1h, 6h, 1d in seconds
        self.granularity_names = {3600: "1h", 21600: "6h", 86400: "1d"}
    ...
    async def run_all_tests(self):
        """Run OrderBookStrategy tests for all combinations of time periods and granularities."""
        print("🚀 Starting OrderBookStrategy validation tests...")
    ...
    def analyze_results(self):
        ...
        if successful_tests > 0:
            ...
            for days in sorted(set(r['days'] for r in successful_results)):
                ...
                print(f"{days:3d} days: {avg_signals:5.1f} signals, {avg_trades:4.1f} trades avg")
            print()
        ...
        if failed_tests > 0:
            ...
            for result in self.results:
                if result['status'] != 'success':
                    print(f"{result['days']:3d} days, {result['granularity_name']:2s}: {result.get('error', 'Unknown error')}")
            print()
    ...
    def save_results(self, filename="orderbook_validation_results.json"):
...
async def main():
    """Main test execution."""
    async with OrderBookStrategyValidator() as validator:
        # Run all tests
        ...
        # Final summary
        print("🎉 OrderBookStrategy Validation Complete!")
