...
import { useLivePortfolio } from '@/hooks/useTrading';
...
export function ManualTradeSection({ symbols }: ManualTradeSectionProps) {
  const { data: portfolio } = useLivePortfolio(true);
  ...
  const getAvailableBalance = () => {
    ...
    if (side === 'buy') {
      // Return USD balance
      return portfolio.cash_balance || portfolio.available_balance_usd || 0;
    } else {
      ...
      if (Array.isArray(positions)) {
         position = positions.find((p: any) => p.symbol === selectedSymbol || p.asset === selectedSymbol.split('-')[0]);
      } else {
         position = positions[selectedSymbol] || Object.values(positions).find((p: any) => p.symbol === selectedSymbol || p.asset === selectedSymbol.split('-')[0]);
      }
      
      if (position) {
          return parseFloat(position.quantity || position.balance_crypto || 0);
      }
      return 0;
    }
  };
    ...
    <Card>
      ...
      {isVisible && (
          <CardContent className="space-y-4">
            ...
            >
                {isLoading ? 'Executing...' : `${side.toUpperCase()} ${selectedSymbol}`}
            </Button>
